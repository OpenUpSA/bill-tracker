  ...
  useEffect(() => {
    ...
    if (groupBillsBy === "status") {
      ...
      statuses.forEach((status) => {
        const billsByStatus = filteredBills.filter(
          ...
          <div className="event-tooltip">
            ...
            <div className="tooltip-body mt-4">
              ...
              <Row>
                <Col xs="4">House:</Col>
                <Col xs="8">
                  {houses.find((h) => h.house === event.house)?.name}
                </Col>
              </Row>
              <Row>
                <Col xs="4">Type:</Col>
      ...
      <header className="p-5">
        <Container fluid>
          <Row className="justify-content-between">
            <Col md={8}>
              <h1>
                <FontAwesomeIcon icon={faPenNib} /> Progress of Bills
              </h1>
              <span>
                How long does it take for a bill to go from being introduced to
                  ...
                  >
                    ...
                    <Dropdown.Menu>
                      {viewOptions.map((option, index) => (
                        <Dropdown.Item
                          key={index}
                          onClick={() => handleViewOptionsChange(option.value)}
                        >
                          <FontAwesomeIcon
                            icon={
                    ...
                    <Dropdown.Menu>
                      {statusOptions.map((option, index) => (
                        <Dropdown.Item
                          key={index}
                          onClick={() =>
                            setSelectedStatuses((prevSelectedOptions) => {
            ...
            <Col>
              <div className="bill-tracker-viz">
                  ...
                  billGroups?.map((group, index) => {
                    return (
                      <div className="bill-group" key={`bill-group-${index}`}>
                        <h2>
                          {lookup.groupby[group.name]} ({group.bills.length})
                        </h2>
                          ...
                          group?.bills?.map((bill, index) => {
                            return (
                              <div className="bill-row" key={`bill-${index}`}>
                                <div
                                  className="bill-title-container"
                                  onClick={() => {
                                      ...
                                      >
                                        <div
                                          ...
                                          style={{
                                            ...
                                            backgroundColor: houses.find(
                                              (h) =>
                                                h.house ==
                                                bill.houses[index][0].house
                                            )?.color,
                                          }}
                                          // onMouseOver={() => handleMouseOver(null, bill)}
                                          // onMouseOut={handleMouseOut}
                                        >
                                          {bill.houses[index].map(
                                            (event, index) => {
                                              const eventPosition =
                                                getEventPosition(
                                                  event.date,
                ...
                {maxDays > 0 && (
                  <div
                    className="xAxis"
                    style={{ width: `${maxDays * daySizeinPx}px` }}
                  >
