...
if (!connectionStringPMG || !connectionStringPA) {
  console.error('❌ PA_DATABASE_URL or PMG_DATABASE_URL not set in .env file');
  process.exit(1);
}

const clientPMG = new Client({
  connectionStringPMG,
  ssl: {
  ...
  try {
    ...
    const query = `
      SELECT
        ...
        "Event - Meeting"."date" AS "event_meeting_date",
